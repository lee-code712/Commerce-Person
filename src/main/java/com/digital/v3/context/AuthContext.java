...
public class AuthContext {
	...
	/* token 생성 */
	public String generateToken (long personId) throws Exception {
		
		Auth auth = new Auth();
        auth.setPersonId(personId);
        
		String responseJson = requestPOST(baseUrlKey, "/rest/auth/generateToken", auth);
		auth = (Auth) jsonToObject(responseJson, Auth.class);
	...
	public void deleteToken (String token) throws Exception {
		...
		requestPOST(baseUrlKey, "/rest/auth/deleteToken", null, token);
	}
